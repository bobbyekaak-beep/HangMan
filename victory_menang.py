...
class Screen9Victory(tk.Frame):
    ...
    def populate_data(self, data):
        ...
        sisa_waktu = data.get("sisa_waktu", 75)   # Default 01:15
        hp_player = data.get("hp_player", 80)     # Default HP 80
        tebakan_benar = data.get("tebakan_benar", 7)

        # ── 1. BANNER ATAS (VICTORY) ───────────────────────────
        ...
        # ── 4. RINCIAN SKOR BONUS (STATS DETAIL) ────────────────
        stats_frame = tk.Frame(self, bg="#F0F4F6")
        stats_frame.pack(fill="x", padx=45, pady=15)

        m, s = divmod(sisa_waktu, 60)
        
        # Format Data: (Ikon Baru, Warna Ikon, Ukuran Font, Nama Teks, Nilai Angka, Nilai Bonus)
        # Ikon target kuning dinaikkan ke ukuran 18 agar lebih besar
        items = [
            ("⏱️", "#4CAF50", 14, "Sisa Waktu", f"{m:02d}:{s:02d}", "+70 Pts"),   # Ikon Jam warna Hijau
            ("❤️", "#E53935", 14, "Sisa HP Kamu", f"{hp_player}/100", "+60 Pts"), # Ikon Hati warna Merah
            ("🎯", "#FFB300", 18, "Tebakan Benar", f"{tebakan_benar} Huruf", "+20 Pts") # Ikon Target warna Kuning (Lebih Besar)
        ]
