...
class HangmanLogic:
    def __init__(self, user_id=None):
        ...
        # ===========================
        self.data_soal = ambil_pertanyaan(1)
        ...
        if not self.daftar_kata:
            raise ValueError(f"Tidak ada soal untuk level {1}")
        ...
        self.hp_player = 100
        self.hp_player_max = 100

        self.hp_musuh = cfg["hp_musuh"]
        self.hp_musuh_max = cfg["hp_musuh"]
    ...
    def hitung_damage_kata(self):
        ...
        self.damage_per_huruf = (
            self.hp_musuh_max / len(self.huruf_unik)
        )
    ...
    def tebak_huruf(self, huruf):
        ...
        if (
            huruf in self.huruf_ditebak
            or self.game_selesai
            or self.pindah_kata_berjalan
        ):
        ...
        if huruf in self.kata_rahasia:
            ...
            self.hp_musuh = max(
                0,
                self.hp_musuh - self.damage_per_huruf
            )
    ...
    def cek_status_game(self):
        ...
        kata_terbuka = all(
            c in self.huruf_ditebak
            for c in self.huruf_unik
        )
    ...
    def hitung_skor(self):
        ...
        return max(
            0,
            base + bonus_waktu + bonus_hp - penalti
        )
