...
class Screen6Gameplay(tk.Frame):
    def __init__(self, parent):
        ...
        # hitung jumlah huruf unik untuk menentukan damage proporsional
        # Contoh: "HARIMAU" punya 7 huruf unik. 100 / 7 = ~14.28 damage per huruf
    ...
    def setup_ui(self):
        ...
        self.lbl_timer = tk.Label(hud, text="⏱  01:30", bg="#F0F4F6", font=("Arial", 15, "bold"), fg="#3E2723")
    ...
    def proses_tebakan(self, h):
        ...
        if h in self.kata_rahasia:
            ...
            # Pengurangan HP dinamis
            self.hp_musuh = max(0, self.hp_musuh - self.damage_per_huruf)
            
            # Jika semua huruf unik sudah terbuka, pastikan HP musuh mutlak 0 (menghindari sisa desimal pembahan)
    ...
    def cek_kondisi(self):
        ...
        # Syarat menang: Kata harus terbuka DAN hp musuh sudah 0
        if kata_terbuka and self.hp_musuh <= 0:
            ...
            if hasattr(self.parent, "set_hasil_game"):
                self.parent.set_hasil_game(
                    kata=self.kata_rahasia, skor=skor, sisa_waktu=self.sisa_waktu, hp_player=self.hp_player,
                    tebakan_benar=self.tebakan_benar, tebakan_salah=self.tebakan_salah, huruf_ditebak=set(self.huruf_ditebak), mode="victory"
                )
            ...
            if hasattr(self.parent, "set_hasil_game"):
                self.parent.set_hasil_game(
                    ...
                    tebakan_benar=self.tebakan_benar, tebakan_salah=self.tebakan_salah, huruf_ditebak=set(self.huruf_ditebak), mode="defeat"
                )
            self.after(1200, lambda: self.parent.switch_screen("screenresult.py"))
...
if __name__ == "__main__":
    class MockParent(tk.Tk):
        def __init__(self):
            super().__init__()
            self.title("Game Hangman Standar")
